...
- ✅ `store/emailComposerStore.ts` - AST構造対応
  ...
  - ✅ `headerHtml` / `footerHtml` - マーカー前後の固定HTML

#### コンポーネント挿入マーカー ✅ (2026-01-17)
- ✅ `<!--ここにコンポーネントを入れる-->` マーカー対応
- ✅ ヘッダー/フッター領域の分離・固定表示
- ✅ マーカー範囲内のみコンポーネント編集可能
- ✅ ルール説明: `docs/visual-editor-rules.md`

#### 自動data-editable付与 ✅ (2026-01-17)
- ✅ スニペット追加時にテキスト要素へ自動付与
- ✅ 対象タグ: h1-h6, p, div, span, td, th, li, label
- ✅ 子要素のないリーフ要素のみが対象
- ✅ 既存属性は上書きせず保持
...
## Visual Editor入力同期修正 ✅ (2026-01-17)

### 問題
- Visual Editorで文字入力中にリアルタイムでMonacoに同期され、入力が干渉される
- 日本語入力等で文字が消える・重複する

### 解決策
入力中は同期せず、**編集完了時のみ**Storeに同期する。
...
### 実装
- **ファイル**: `components/email-composer/VisualPreviewEditor.tsx`
- `input`イベントでの即時同期を削除
- `blur`/`keydown(Escape)`/選択解除時のみ`syncToStore()`を呼び出す
