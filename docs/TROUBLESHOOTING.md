...
## Monaco-Visual Editor同期問題 （✅ 完全解決済み）

### 症状
- MonacoでHTMLを編集するとコンテンツが破損する
- ネストされた`<div>`タグが途中で切れる
- Visual Editorに変更が反映されない
...
正規表現の非貪欲マッチ `[\s\S]*?` がネストされたタグで問題を起こしていました：
...
ネストされた`</div>`がある場合、最初の`</div>`でマッチが停止：
```html
...
2. 範囲内で`lastIndexOf('</div>')`で最後の閉じタグを見つける
...
正規表現は**括弧の対応を数えられない**（正規言語 vs 文脈自由言語の理論的制約）。そのため、ネスト可能なタグ（div, span, table等）の正確なマッチングは不可能。

コメントマーカーはネスト不可能なため、正規表現で安全に範囲を特定できます。
...
**Last Updated**: 2026-01-17 (Monaco-Visual Editor同期問題追加)
